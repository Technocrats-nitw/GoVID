...
''' 
DOCUMENTATION :
The file has following objects:
    class FaissTopK(object):# does similarity search and finds the top K similar answers 
    class RetreiveQADoc(object):# the object of this class shall be used to get the answer from the question
    class GenerateQADoc(object):#optional feature ... use of the same above model to get the question of the answer : didn't perform well though
'''


#loads weight in feed forward network and the BERT from weight files
def load_weight(model, bert_ffn_weight_file=None, ffn_weight_file=None):
    if bert_ffn_weight_file:
        ...
        model.get_layer('q_ffn').set_weights([loader('q_ffn/ffn_layer/kernel/.ATTRIBUTES/VARIABLE_VALUE'),
             loader('q_ffn/ffn_layer/bias/.ATTRIBUTES/VARIABLE_VALUE')])
        
        model.get_layer('a_ffn').set_weights([loader('a_ffn/ffn_layer/kernel/.ATTRIBUTES/VARIABLE_VALUE'),
             loader('a_ffn/ffn_layer/bias/.ATTRIBUTES/VARIABLE_VALUE')])


class QAEmbed(object):
    
    def __init__(self,hidden_size=768,dropout=0.2,residual=True,pretrained_path=None,batch_size=128,max_seq_length=256,
            ffn_weight_file=None,bert_ffn_weight_file=None,load_pretrain=True,with_question=True,with_answer=True):
        ...
        layer_ind = -2 if bert_ffn_weight_file is None else -1
        
        #specifying model
        self.model = MedicalQAModelwithBert(hidden_size=768,dropout=0.2,residual=True,config_file=config_file,checkpoint_file=checkpoint_file,layer_ind=layer_ind)
        self.batch_size = batch_size
        self.tokenizer = FullTokenizer(os.path.join(pretrained_path, 'vocab.txt'))
    ...
    def _type_check(self, inputs):
        if inputs is not None:
            if isinstance(inputs, str):
                ...
                raise TypeError('inputs are supposed to be str of list of str, got {0} instead.'.format(type(inputs)))
...
class FaissTopK(object):
    # does similarity search and finds the top K similar answers 
...
# the object of this class shall be used to get the answer from the question
class RetreiveQADoc(object):
    
    def __init__(self,pretrained_path=None,ffn_weight_file=None,bert_ffn_weight_file='models/bertffn_crossentropy/bertffn',embedding_file='qa_embeddings/bertffn_crossentropy.zip'):
        super(RetreiveQADoc, self).__init__()
        #obtaining the embedding of input vector
        self.qa_embed = QAEmbed(pretrained_path=pretrained_path,ffn_weight_file=ffn_weight_file,bert_ffn_weight_file=bert_ffn_weight_file)
        #feeding to FCNN get the simlarity embeddings
...
#optional feature ... use of the same above model to get the question of the answer : didn't perform well though
class GenerateQADoc(object):
    def __init__(self,pretrained_path='models/pubmed_pmc_470k/',ffn_weight_file=None,bert_ffn_weight_file='models/bertffn_crossentropy/bertffn',
                 gpt2_weight_file='models/gpt2',embedding_file='qa_embeddings/bertffn_crossentropy.zip'):
        
        super(GenerateQADoc, self).__init__()
        
        tf.compat.v1.disable_eager_execution() #GPT2 is in TF1.x.. disabling eager execution of TF2.x to support it
        ...
        #return gpt2 head model for answer generation
        gpt2_model_fn = gpt2_estimator.get_gpt2_model_fn(accumulate_gradients=5,learning_rate=0.1,length=512,batch_size=self.batch_size,temperature=0.7,top_k=0)
        
        hparams = gpt2_estimator.default_hparams()
        #loading hyperparams of gpt
        with open(os.path.join(gpt2_weight_file, 'hparams.json')) as f:
            hparams.override_from_dict(json.load(f))
        self.estimator = tf.estimator.Estimator(gpt2_model_fn,model_dir=gpt2_weight_file,params=hparams,config=config)
        ...
        with self.embed_sess.as_default():
            self.qa_embed = QAEmbed(pretrained_path=pretrained_path,ffn_weight_file=ffn_weight_file,bert_ffn_weight_file=bert_ffn_weight_file,with_answer=False,load_pretrain=False)
    ...
    def predict(self, questions, search_by='answer', topk=5, answer_only=False):
        embedding = self.qa_embed.predict(questions=questions, dataset=False).eval(session=self.embed_sess)
        
        if answer_only:
            topk_answer = self.faiss_topk.predict(embedding, search_by, topk, answer_only)
        else:
            topk_question, topk_answer = self.faiss_topk.predict(
                embedding, search_by, topk, answer_only)

        gpt2_input = self._get_gpt2_inputs(questions[0], topk_question, topk_answer)
        gpt2_pred = self.estimator.predict(lambda: gpt2_estimator.predict_input_fn(inputs=gpt2_input, batch_size=self.batch_size, checkpoint_path=self.gpt2_weight_file))
        raw_output = gpt2_estimator.predictions_parsing(gpt2_pred, self.encoder)
...
if __name__ == "__main__":
    gen = GenerateQADoc()
    print(gen.predict('i am stressed'))
